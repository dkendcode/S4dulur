...
// count user profile
	...
	for ($j = 0; $j < $TotalReg; $j++) {
		$totaldetail = $getprofile[$j];
		$ponlogin = $totaldetail['on-login'];
		$gettotlist = explode(",", $ponlogin);
		if($gettotlist[8]!='Tree' || $totaldetail['name']=='default'){
			$totalList += 1;
		}
	}
}
...
<table id="tFilter" class="table table-bordered table-hover text-nowrap">
  ...
  <tr> 
	...
	?></th>
		...
		<th class="align-middle"><?= $_telegram_report ?></th>
    </tr>
  </thead>
  <tbody>
<?php
...
for ($i = 0; $i < $TotalReg; $i++) {
  ...
  $monid = $monexpired['.id'];
	...
	$getlist = explode(",", $ponlogin);
	if($getlist[8]!='Tree' || $pname=='default'){
		echo "<tr>";
		?>
		<td style='text-align:center;'><i class='fa fa-minus-square text-danger pointer' onclick="if(confirm('Are you sure to delete profile (<?= $pname; ?>)?')){window.location='./?remove-user-profile=<?= $pid; ?>&remove-user-profile-monitor=<?= $monid;?>&session=<?= $session; ?>'}else{}" title='Remove <?= $pname; ?>'></i>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp
		<?php
