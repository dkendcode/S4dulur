...
if (!isset($_SESSION["mikhmon"])) {
  ...
  $gettelegram = explode(",", $ponlogin)[7];
  ...
  if (isset($_POST['name'])) {
    ...
    // Pendapatan / Income
    $getPendapatan = ';[:local getPendapatan [:put ([/system script get [find name=ReportPendapatan] source] + '.$price.')]]';
    $setPendapatan = ';[/system script set source="$getPendapatan" ReportPendapatan ]';
    $Pendapatan =  ';[:local Pendapatan [:put [/system script get [find name=ReportPendapatan] source]]]';
    if(!$API->comm("/system/script/print", array("?name" => "ReportPendapatan"))){$usetlgrm="No";}

    $parent = ($_POST['parent']);

    $onlogin = ':put (",'.$expmode.',' . $price . ',' . $validity . ','.$sprice.',,' . $getlock . ','. $usetlgrm .',Simple,,,,,,,"); {:local price ("' . $price . '");:local date [ /system clock get date ];:local year [ :pick $date 7 11 ];:local month [ :pick $date 0 3 ];:local comment [ /ip hotspot user get [/ip hotspot user find where name="$user"] comment]; :local ucode [:pic $comment 0 2]; :if ($ucode = "vc" or $ucode = "up" or $comment = "") do={ /sys sch add name="$user" disable=no start-date=$date interval="' . $validity . '"; :delay 2s; :local exp [ /sys sch get [ /sys sch find where name="$user" ] next-run]; :local getxp [len $exp]; :if ($getxp = 15) do={ :local d [:pic $exp 0 6]; :local t [:pic $exp 7 16]; :local s ("/"); :local exp ("$d$s$year $t"); /ip hotspot user set comment=$exp [find where name="$user"];}; :if ($getxp = 8) do={ /ip hotspot user set comment="$date $exp" [find where name="$user"];}; :if ($getxp > 15) do={ /ip hotspot user set comment=$exp [find where name="$user"];}; /sys sch remove [find where name="$user"]';
    ...
    if($token == "" && $cid == ""){
      if ($expmode == "rem") {
        ...
        $onlogin = ':put (",,' . $price . ',,,noexp,' . $getlock . ',No,Simple,,,,,,,")' . $lock;
      } else {
        $onlogin = "";
      }
    }elseif(!$API->comm("/system/script/print", array("?name" => "ReportPendapatan"))){
      if ($expmode == "rem") {
        $onlogin = $onlogin . $lock ."}}";
        $mode = "remove";
      } elseif ($expmode == "ntf") {
      ...
      if($usetlgrm == "Yes"){
        if ($expmode == "rem") {
          $onlogin = $onlogin . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
          $mode = "remove";
        } elseif ($expmode == "ntf") {
          $onlogin = $onlogin . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
          $mode = "set limit-uptime=1s";
        } elseif ($expmode == "remc") {
          $onlogin = $onlogin . $record . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
          $mode = "remove";
        } elseif ($expmode == "ntfc") {
          $onlogin = $onlogin . $record . $lock . $getPendapatan . $setPendapatan . $Pendapatan . $telegrambot . "}}";
          $mode = "set limit-uptime=1s";
        ...
        if ($expmode == "rem") {
          $onlogin = $onlogin . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
          $mode = "remove";
        } elseif ($expmode == "ntf") {
          $onlogin = $onlogin . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
          $mode = "set limit-uptime=1s";
        } elseif ($expmode == "remc") {
          $onlogin = $onlogin . $record . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
          $mode = "remove";
        } elseif ($expmode == "ntfc") {
          $onlogin = $onlogin . $record . $lock . $getPendapatan . $setPendapatan . $Pendapatan . "}}";
          $mode = "set limit-uptime=1s";
...
<form autocomplete="off" method="post" action="">
  <div>
    ...
    <a class="btn bg-danger" href="./?remove-user-profile=<?= $pid; ?>&remove-user-profile-monitor=<?= $monid;?>&session=<?= $session; ?>"><i class="fa fa-minus-square"></i> <?= $_remove; ?></a>
  </div>
<table class="table">
  <tr>
  ...
  <tr id="tlgrm" style="display:none;">
    <?php if($token != "" && $cid != "" && $API->comm("/system/script/print", array("?name" => "ReportPendapatan"))):?>
    <td><?= $_telegram_report?></td><td>
      <select class="form-control" id="usetlgrm" name="usetlgrm" required="1">
        <option value="<?= $gettelegram;?>"><?= $gettelegram;?></option>
